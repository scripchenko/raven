...
Telegram, WhatsApp и MAX по умолчанию используют общий bundled-звук Lantern по событию `NotificationReceived`; для каждого типа сервиса можно выбрать режим «Lantern» или «Родной». В режиме Lantern встроенный web-звук при необходимости отключается пользователем вручную. Приложение не меняет web-настройки и не заглушает WebView2, поэтому voice messages, calls, video и другое media продолжают работать независимо.
