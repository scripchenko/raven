...
Этапы 1–3 находятся в `main`. В текущей рабочей ветке реализован **этап 4 — системный трей, уведомления и индикаторы активности**:
...
Все адреса и отдельные списки разрешённых доменов находятся в централизованном каталоге. Gmail и режимы управления памятью в этап 4 не входят.
...
Закрытие главного окна по умолчанию скрывает его в область уведомлений; команда «Выход» завершает приложение и освобождает WebView2 и `NotifyIcon`. Режим «Не беспокоить» и заглушение аккаунта подавляют новые popup-уведомления, но не отключают сервис и не очищают его индикатор активности. До трёх собственных popup показываются в правом нижнем углу без активации окна; `ShowBalloonTip` остаётся резервным вариантом. Общая новая активность дополнительно отображается красным overlay на кнопке приложения в панели задач.
...
Явный Exit идемпотентен: контекстное меню и `NotifyIcon` удаляются до освобождения WebView2, новые UI-события блокируются, popup и их таймеры закрываются, а настройки сохраняются асинхронно до вызова `Application.Shutdown`. Обычный крестик продолжает только скрывать окно в трей.
