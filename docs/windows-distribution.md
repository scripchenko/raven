...
Stage 8.1 формирует folder-based self-contained publish для `win-x64` и минимальный per-user installer raven на Inno Setup 6. Внутренние имена проекта и исполняемого файла остаются `UnifiedMessenger.App`.
...
Installer устанавливает raven для текущего пользователя в сохранённый совместимый каталог:

```text
%LOCALAPPDATA%\Programs\Lantern
```

Создаётся обязательный ярлык raven в Start Menu. Ярлык на Desktop является опциональным и по умолчанию выключен.

Будущая версия использует тот же постоянный Inno Setup `AppId` и обновляет файлы в том же install directory. Если raven запущен или скрыт в tray, стандартный Windows Restart Manager предлагает закрыть его перед заменой файлов. Принудительное завершение не используется, а автоматический перезапуск приложения после upgrade отключён.
...
Uninstall удаляет только install directory, shortcuts и запись raven в Installed Apps. Отдельной опции удаления пользовательских данных на этом этапе нет.
...
Если Runtime отсутствует, установка raven останавливается и предлагает открыть официальную страницу Microsoft:

<https://developer.microsoft.com/microsoft-edge/webview2/>

WebView2 bootstrapper или standalone installer в Git и в payload Stage 8.1 не включается. На машине пользователя должен быть установлен Evergreen Runtime; существующая проверка внутри raven остаётся дополнительным fallback.
